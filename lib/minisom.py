...
class MiniSom(object):
    ...
    def _mexican_hat(self, c, sigma):
        ...
        d = 2*sigma*sigma
        return (exp(-p/d)*(1-2/d*p)).T #type: ignore
    ...
    def _topographic_error_hexagonal(self, data):
        ...
        te = 1 - mean(b2mu_neighbors) #type: ignore
        return te
    ...
    def labels_map(self, data, labels):
        ...
        for position in winmap:
            winmap[position] = Counter(winmap[position]) #type: ignore
        return winmap 
...
class TestMinisom(unittest.TestCase):
    ...
    def test_euclidean_distance(self):
        ...
        assert_array_almost_equal(d, [[1.41421356, 1.41421356], #type: ignore
    ...
    def test_cosine_distance(self):
        ...
        assert_array_almost_equal(d, [[1., 1.], #type: ignore
                                      [1., 1.]])
    ...
    def test_bubble(self):
        ...
        assert bubble[2, 2] == 1
        assert sum(sum(bubble)) == 1 #type: ignore
    ...
    def test_labels_map(self):
        ...
        assert labels_map[(2, 3)]['a'] == 1 #type: ignore
        assert labels_map[(1, 1)]['b'] == 1 #type: ignore
        with self.assertRaises(ValueError):
    ...
    def test_use_epochs_variables(self):
        ...
        iterations = _build_iteration_indexes(len_data, num_epochs,
                                              ...
                                              use_epochs=True)
        assert num_epochs*len_data == len(iterations) #type: ignore

        # checks whether all epochs share the same order of indexes
        first_epoch = iterations[0:len_data] #type: ignore
        for i in range(num_epochs):
            i_epoch = iterations[i*len_data:(i+1)*len_data] #type: ignore
    ...
    def test_callable_activation_distance(self):
        ...
        som1 = MiniSom(5, 5, 2, sigma=1.0, learning_rate=0.5,
                       activation_distance=euclidean, random_seed=1) #type: ignore
        som1.train_random(data, 10)
        som2 = MiniSom(5, 5, 2, sigma=1.0, learning_rate=0.5, random_seed=1)
        som2.train_random(data, 10)
        # same state after training
